 ...
 * 
 * **/
if(!function_exists('allianz_woocs_currency_switcher')){
	function allianz_woocs_currency_switcher($args = []){
	...
	<ul class="cms-woocs-menu cms-dropdown dropdown-<?php echo esc_attr($args['dropdown_pos']);?> cms-touchedside">
		<li class="<?php echo allianz_nice_class(array_merge($item_classes, ['current-item'])); ?>">
			<?php 
				foreach ($all_currencies as $key => $currency) {
					...
					?>
					<a href="#" class="<?php echo allianz_nice_class($link_classes); ?>" data-currency="<?php echo esc_attr($currency['name']) ?>" title="<?php echo esc_attr($currency['name']) . ', ' . esc_attr($currency['symbol']) . ' ' . esc_attr($currency['description']) ?>" onclick="event.preventDefault()">
	        			...
	        			<?php if($show_text === 'yes') { ?>
	        				<span class="cms-hidden-tablet"><?php echo esc_html($option_txt); ?></span>
			...
			<ul class="dropdown cms--touchedside">
				<?php
					foreach ($all_currencies as $key => $currency) {
			            ...
			            if($current_currency !== $key){
			        ?>
			        	<li class="<?php echo allianz_nice_class($item_classes); ?>">
			        		<a href="#" class="<?php echo allianz_nice_class($sub_link_classes); ?>" data-currency="<?php echo esc_attr($currency['name']) ?>" title="<?php echo esc_attr($currency['name']) . ', ' . esc_attr($currency['symbol']) . ' ' . esc_attr($currency['description']) ?>" class="">
			        			<?php //if($show_flags === 'yes') { ?>
			        				<img src="<?php echo esc_attr($flag_url) ?>" alt="<?php echo esc_attr($currency['name']) . ', ' . esc_attr($currency['symbol']) ?>" class="pr-10" />
			        			<?php //} ?>
