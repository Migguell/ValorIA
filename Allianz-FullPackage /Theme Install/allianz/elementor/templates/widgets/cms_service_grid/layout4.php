...
switch ($settings['link1_type']) {
    case 'page':
        $_link1_page = !empty($link1_page) ? get_page_by_path($link1_page, OBJECT) : [];
        $url  = !empty($_link1_page) ? get_permalink($_link1_page->ID) : '#';
        break;
...
$widget->add_render_attribute( 'link1_text', [
    'class' => [
        'cms-link text-15 font-700 pt-25 pb-5',
        ...
        'd-flex gap-10 align-items-center justify-content-center',
    ],
    'href'  => $url,
...
<?php }
    if(!empty($settings['description_bold_text'])){ ?>
...
<?php }
    if(!empty($settings['link1_text'])): ?>
        <a <?php etc_print_html( $widget->get_render_attribute_string( 'link1_text' ) ); ?>><?php
            // text
            etc_print_html($settings['link1_text']);
            // icon
            allianz_elementor_button_icon_render();
        ?></a>
<?php 
    endif;
$posts_data['grid_custom_content'] = ob_get_clean();
?>
