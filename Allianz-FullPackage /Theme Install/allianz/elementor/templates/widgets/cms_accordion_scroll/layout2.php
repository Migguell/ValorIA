...
$widget->add_render_attribute('wrap', [
	'class' => [
		...
		'cms-scroller-'.$settings['scroll_mode']
	]
]);
?>
<div <?php etc_print_html($widget->get_render_attribute_string('wrap')); ?>>
	...
	foreach ( $accordions as $key => $value ):
		...
		$widget->add_render_attribute($item_inner_key, [
			'class' => [
				...
				'h-100',
				'p-50 p-laptop-40 p-lr-tablet-20'
			]
		]);
		// Items Inner2
		$item_inner_key_2 = $widget->get_repeater_setting_key( 'ac_items_inner_2', 'cms_accordion_scroll', $key );
		$widget->add_render_attribute($item_inner_key_2, [
			'class' => [
				'cms-accs---item',
				'container',
				'overflow-hidden',
		...
		switch ($settings['scroll_mode']) {
			...
			default:
				$widget->add_render_attribute( $content_key, [
					'style' => [
						'--cms-sticky--mousewheel-height:calc(100% - var(--cms-sticky-top, 60px));'
					]
				]);
				if($count > 0){
					$widget->add_render_attribute($item_key, [
						'class' => [
							//'cms-scroller-'.$settings['scroll_mode'].'-item',
						]
					]);
				}
				break;
	...
	<div <?php etc_print_html( $widget->get_render_attribute_string( $item_key ) ); ?>>
		<div <?php etc_print_html( $widget->get_render_attribute_string( $item_inner_key ) ); ?>>
			<div <?php etc_print_html( $widget->get_render_attribute_string( $item_inner_key_2 ) ); ?>>
	    	...
	    	?></h3>
			  <div <?php etc_print_html( $widget->get_render_attribute_string( $content_key ) ); ?>>
					...
					<div class="col-6 col-mobile-extra-12">
						<div class="inner pl-70 pl-tablet-extra-0 cms-sticky" style="--cms-sticky-top:0;"><?php 
			  			ob_start();
				  		?>
